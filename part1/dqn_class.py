...
class DQNAlgoAgent:
    ...
    def __init__(self, env, bin_array, learning_rate=3e-4, gamma=0.99, buffer_size=10000):
        ...
        self.gamma = gamma
        self.replay_buffer = ReplayBuffer(buffer_max_size=buffer_size)
        # GPU usage	
        ...
        if isinstance(env.action_space.sample(),int):
            ...
            # Target network
            self.target_model = FullConnectedNN(env.observation_space.shape, env.action_space.n).to(self.device)
        # Analog action space
        else:
            # Q network
            ...
            # Target network
            self.target_model = FullConnectedNN(env.observation_space.sample().shape, int(math.pow(len(bin_array)-1,env.action_space.sample().shape[0]))).to(self.device)
        
        # Switch Q network to train mode
        self.model.train()
        # Sync parameters of target network
    ...
    def calculate_loss(self, batch_tuple):
        ...
        # done_flags = torch.FloatTensor(done_flags)
        ...
        # Forward pass from target network
        next_qvalue = self.target_model.forward(next_states)
        # Argmax
    ...
    def train_model(self, batch_size):
        batch_data = self.replay_buffer.get_samples(batch_size)
